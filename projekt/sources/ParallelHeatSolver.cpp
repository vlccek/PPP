...
void ParallelHeatSolver::initGridTopology()
{
    ...
    MPI_Cart_coords(gridComm, my_grid_rank, ndims, my_coords);

    int middle_col_index = dims[0] / 2;
    int color = (my_coords[0] == middle_col_index) ? 1 : MPI_UNDEFINED;
    ...
    MPI_Comm_split(MPI_COMM_WORLD, 0, mWorldRank, &avgTempComm);
}
